...
ショートコードで表示されたタブ付きリストは、上部の「フィルターパネル」から条件を切り替えられます。基本的な操作手順は次のとおりです。

1. ショートコードを配置したページ（または管理画面のプレビュー）を開き、一覧の上に表示されるフィルターグループを確認します。フィルターを追加していない場合はパネル自体が表示されません。
2. それぞれのフィルターグループは登録時の「グループ名」が見出しになります。グループ名を指定していない場合は「フィルター1」「フィルター2」のように自動命名されます。
3. グループ名の直下には「すべて」を含む複数のボタンが並びます。各ボタンがタブに相当し、クリックすると即座に対象の値だけが一覧に残ります。

#### [フィルター1] の [D] を選択する手順

例として「フィルター1」に候補 A/B/C/D が生成されている場合、[D] を選択するには以下の操作を行います。

1. フィルターパネル内で「フィルター1」という見出しを探します（グループ名を変更している場合はその名称が表示されます）。
2. 「フィルター1」の下に並ぶボタンの中から `[D]` と表示されたものをクリックします。ボタンが青色（既定テーマ）で強調表示され、[D] の条件が適用されます。
3. 一覧が自動的に再描画され、「フィルター1 = D」に一致するデータだけが表示されます。別の条件に切り替えたい場合は、同じグループ内の他のボタンや「すべて」をクリックしてください。

選択中の条件は URL には自動保存されないため、リンク共有時に状態を固定したい場合は「出力設定 → パラメーター制御」でフィルター ID を公開し、ショートコードの引数やクエリパラメーター（例: `?filter_filter1=D`）を利用してください。各フィルターの設定欄には「フィルターID」と実際に利用するパラメーター名（`filter_<ID>` 形式）が表示されるため、その文字列をそのままクエリに指定します（例: フィルターID が `filter1` の場合は `?filter_filter1=D`）。
